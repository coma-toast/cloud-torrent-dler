		...
		for _, unsortedItem := range unsortedItems {
			...
			if isAVideo {
				...
				if unsortedItem.ShowID != 0 {
					...
					path := fmt.Sprintf("%s/%s/%s%s", conf.DownloadDestination, conf.CompletedFolders[0], unsortedItem.TVShowName, unsortedItem.FolderPath)
		...
		for _, downloadFolder := range conf.CompletedFolders {
			...
			for _, item := range list {
				...
				if isAVideo {
					...
					localPath := fmt.Sprintf("%s/%s", conf.DownloadDestination, item.FolderPath)
